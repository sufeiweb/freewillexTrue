var GLOBAL_VAR = {
    ...
    button_down: false,
    init: false,
    url: "www.baidu.com" // 获取所有数据的地址(ajax)
};
/**
 * 周期图
 */
...
function switch_period(period) {
    ...
    if (wsInit > 0) {
        ...
        periodTemp == "DAY" ? "" : (_.isUndefined(periodTemp) ? (WS.params['ticker']).push({"code": Config.code, "period": 'MIN1'}) : (WS.params['ticker']).push({"code": Config.code, "period": periodTemp}));
