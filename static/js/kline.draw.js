var GLOBAL_VAR = {
    ...
    button_down: false,
    init: false,
    url: "/api/market/kLine" // 获取所有数据的地址(ajax)
};
/**
 * 周期图
 */
...
ChartManager.prototype.__construct = function () {
    ...
    // this._language = results.lang == 'en' ? 'en-us' : results.lang;
...
DarkTheme.prototype.__construct = function () {
    this._colors = [];
    this._colors[Theme.Color.Positive] = "#19b34c";// 涨 #19b34c
    this._colors[Theme.Color.Negative] = "#990e0e"; // 跌 #990e0e
    ...
    this._colors[Theme.Color.TextPositive] = "#1bd357";// #1bd357
    this._colors[Theme.Color.TextNegative] = "#ff6f5e";// #ff6f5e
...
LightTheme.prototype.__construct = function () {
    this._colors = [];
    this._colors[Theme.Color.Positive] = "#53b37b"; // 涨#53b37b
    this._colors[Theme.Color.Negative] = "#db5542"; // 跌#db5542
    ...
    this._colors[Theme.Color.Grid4] = "#aaa";
    this._colors[Theme.Color.TextPositive] = "#53b37b"; // 涨#53b37b
    this._colors[Theme.Color.TextNegative] = "#db5542"; // 跌#db5542
...
function wsTicker(data) {
    ...
    if (!_.isUndefined(ticker)) {
        ...
        document.title =  '฿:' + "¥" + ticker[4] + "-" + titleTemp[1] + "-" + titleTemp[2];
        var lastDataTime = GLOBAL_VAR.chartMgr.getDataSource("frame0.k0").getLastDate(); // 获取最后一条数据的时间
...
// 重置宽度和高度
function on_size() {
    var innerWidth = window.innerWidth - $("#market_detail").width() - 5;// 返回窗口的文档显示区的宽度
    if ($("#order").height() == null || $("#order").height() == 0) {
        var innerHeight = window.innerHeight - 32;//返回窗口的文档显示区的高度
    } else {
        var innerHeight = window.innerHeight - $("#order").height() - 5;//返回窗口的文档显示区的高度
    }
    ...
    indicatorDialog.css({left: (innerWidth - indicatorDialog.width()) >> 1, top: (innerHeight - indicatorDialog.height()) >> 1});
...
function switch_period(period) {
    ...
    if (wsInit > 0) {
        WS.params['ticker'] = [
            ...
            // {"code": Config.code, "period": "DAY"}
        ];
        periodTemp == "DAY" ? "" : (_.isUndefined(periodTemp) ? (WS.params['ticker']).push({"code": code, "period": 'MIN1'}) : (WS.params['ticker']).push({"code": code, "period": periodTemp}));
